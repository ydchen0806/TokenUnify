...
class MambaEncoder(nn.Module):
    ...
    def forward_features(self, x):
        outs = []
        for i in range(4):  # x.shape: torch.Size([6,1,16,160,160])
            # x = x.float()
...
class Preciever(nn.Module):
    def __init__(self):
        super(Preciever, self).__init__()
        self.conv1 = nn.Conv3d(1, 12, kernel_size=3, stride=2, padding=1)
        self.conv2 = nn.Conv3d(12, 12, kernel_size=3, stride=2, padding=1)
        self.conv3 = nn.Conv3d(12, 1, kernel_size=3, stride=2, padding=1)
        self.mlp = nn.Linear(2*20*20, 4*16*16)
...
class SegMamba(nn.Module):
    def __init__(
        ...
        self.preciever = Preciever()
        # if self.autoregress:
        #     # stop preciver's grad
        #     for param in self.preciever.parameters():
    ...
    def forward(self, x_in, x_gt = None, next_token_gt = None, epoch = 0, mode = 0):
        mode = self.args.auto_mode
        ...
        if x_gt is not None:
            if mode == 0:
                ...
                if (epoch // 100) % 2 == 0:
                    next_token_pred = self.preciever(out)
                    next_token_pred = self.sigmoid(next_token_pred)
                    if next_token_gt is not None:
                        loss = F.mse_loss(next_token_pred.float(), next_token_gt.float())
                        return loss
                    else:
                ...
                next_token_pred = self.sigmoid(next_token_pred)
                out = self.sigmoid(out)
                if x_gt is not None and next_token_gt is not None:
                    loss = F.mse_loss(out, x_gt) + F.mse_loss(next_token_pred.float(), next_token_gt.float())
                    return loss
                else:
                    return out, next_token_pred
            else:
