...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    # if epoch != 0 and epoch % 2 == 0 and misc.is_main_process():
    # if epoch != 0 and epoch !=2 and epoch % 2 == 0 and misc.is_main_process():
    if epoch > 80 and epoch % 5 == 0 and misc.is_main_process():
    # if epoch % 1 == 5 and misc.is_main_process():
            # valid_data, valid_label, valid_affs = dataset.valid_provide()
            valid_data, valid_label, valid_affs = dataset.valid_provide(valid_dataset="ac3")
            ...
            with torch.no_grad():
                ...
                # pred_data = sliding_window_inference(valid_data, (18,160,160), args.batch_size, model, overlap=args.overlap) # parameters: data_raw_valid, model_input_size, valid batch size, model, 0.25
                # pred_data = sliding_window_inference(valid_data, (16,160,160), args.batch_size, model, overlap=args.overlap) # parameters: data_raw_valid, model_input_size, valid batch size, model, 0.25
                ...
                # pred_data = sliding_window_inference(valid_data, (16,160,160), 2, model, overlap=args.overlap) # parameters: data_raw_valid, model_input_size, valid batch size, model, 0.25
