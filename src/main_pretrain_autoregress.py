...
from dataloader.data_provider_pretraining_autoregress import Train as EMDataset
import sys
...
from engine_pretrain_autoregress import train_one_epoch
...
def get_args_parser():
    ...
    parser.add_argument('--auto_mode', default=1, type=int,
                        help='autoregress training or joint training')
    parser.add_argument('--fill_nose', default=1, type=int,
                        help='fill nose or not')
...
def main(args):
    ...
    if args.model == 'unetr':
        model = UNETR(
                ...
                dropout_rate=0.1)
    if args.model == 'segmamba':
        model = SegMamba(in_chans=1, out_chans=1, args=args)
