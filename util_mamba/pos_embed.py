...
def get_3d_sincos_pos_embed(embed_dim, grid_size, cls_token = False):
    ...
    assert len(grid_size) == 3, "grid_size must be a list of 3 elements"
...
def get_3d_sincos_pos_embed_from_grid(embed_dim, grid):
    ...
    emb_d = get_1d_sincos_pos_embed_from_grid(embed_dim // 2, grid[0])  # (D*H*W, D/3)
    emb_h = get_1d_sincos_pos_embed_from_grid(embed_dim // 2, grid[1])  # (D*H*W, D/3)
    emb_w = get_1d_sincos_pos_embed_from_grid(embed_dim // 2, grid[2])  # (D*H*W, D/3)
...
def get_1d_sincos_pos_embed_from_grid(embed_dim, pos):
    ...
    if len(pos.shape) == 3:
        ...
        omega = np.arange(embed_dim // 3, dtype=np.float)
        omega /= embed_dim / 3.
