...
def preserve_patches_and_fill_rest(img, patch_size=(4, 16, 16), fill_with_noise=0):
    ...
    - num_patches_to_preserve (int): Number of patches to preserve.
    ...
    # tokenize, reshape as L * p1 * p2 * p3
    img_token = rearrange(img, '(h p1) (w p2) (d p3) -> (h w d) p1 p2 p3', p1=patch_h, p2=patch_w, p3=patch_d)

    # keep the first num_patches_to_preserve patches
    num_patches_to_preserve = np.random.randint(1, total_patches)
    # img_token = img_token[:num_patches_to_preserve]
    ...
    if fill_with_noise == 1:
        generate_token[num_patches_to_preserve:] = np.random.normal(0, 1, generate_token[num_patches_to_preserve:].shape)
    else:
        generate_token[num_patches_to_preserve:] = 0
    
    # reshape
    img_token = rearrange(img_token, '(h w d) p1 p2 p3 -> (h p1) (w p2) (d p3)', h=h // patch_h, w=w // patch_w, d=d // patch_d)
    generate_token = rearrange(generate_token, '(h w d) p1 p2 p3 -> (h p1) (w p2) (d p3)', h=h // patch_h, w=w // patch_w, d=d // patch_d)
    # next_token = rearrange(next_token, 'p1 p2 p3 -> (h p1) (w p2) (d p3)', p1=patch_h, p2=patch_w, p3=patch_d)
    return generate_token, next_token
	...
	def __getitem__(self, index):
		...
		if self.cfg.trainer.contranstive:
			...
			if random.random() > 0.5:
				imgs1, _, _, _ = self.apply_perturbations(imgs, None, mode=self.per_mode)
			imgs1, next_token = preserve_patches_and_fill_rest(imgs1, patch_size=(4, 16, 16), fill_with_noise=self.args.fill_nose)
			# gt_imgs = imgs[:, self.sub_padding[-1]:-self.sub_padding[-1], self.sub_padding[-1]:-self.sub_padding[-1]]
			imgs1 = imgs1[np.newaxis, ...]
			gt_imgs = gt_imgs[np.newaxis, ...]
			...
			imgs1 = np.ascontiguousarray(imgs1, dtype=np.float32)
			gt_imgs = np.ascontiguousarray(gt_imgs, dtype=np.float32)
			return imgs1, gt_imgs, next_token
