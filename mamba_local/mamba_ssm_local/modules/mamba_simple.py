...
try:
    from ..causal_conv1d_local.causal_conv1d import causal_conv1d_fn, causal_conv1d_update
except ImportError:
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v3":
                ...
                xz_s = xz.chunk(self.nslices, dim=-1)  # xz.shape: torch.Size([6,128,51200])  xz_s: tuple len=5  xz_s[0].shape: torch.Size([6,128,10240])
                xz_s = torch.stack(xz_s,dim=-1)  # xz_s.shape: torch.Size([6,128,10240,5])
                xz_s = xz_s.flatten(-2)  # [6,128,51200]
